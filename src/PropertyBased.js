        ...
        (cmd) => {
            if (size == 0) {
                ...
                return gen.flatmap((a)=>{
                	return jsc.generator.constant(R.concat([[cmd,currentState]])(a));
                });
            }
...
const shrink_sequence = (cmd_seq, state_seq, commands,initialState) => {
    ...
    let validCommands = R.compose((a)=>{return a},R.map((cmd)=>{
 	 ...
 	 const shrink_subseq = (s,shrink,length) => {
 	 	...
 	 	if(!R.isEmpty(s)){
 	 	 	let shrink_seq = R.compose(
 	 	 	 	 	 	    (a)=>{return a},
 	 	 		 	 	 	validCommands,
            	 	 	 	R.map((index) => { return cmd_seq[index]}))(s)
            if(s.length != length){
 	 	 	...
 	 	 	shrink = R.compose(
                	...
                	R.filter(valid_sequence(commands, state_seq, cmd_seq,initialState)),(a)=>{ return a},
                	remove_seq)(s);
